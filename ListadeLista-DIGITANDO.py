...
def exibirMenu(titulo, opcoes, is_submenu=False):
    while True:
        print("\n" + "="*50)
        print(titulo)
        print("Digite o número da opção desejada ou 'voltar' para retornar.\n")
        
        for i, item in enumerate(opcoes, 1):
            if is_submenu:
                num = item
                descricao = enunciados.get(num, "[Enunciado não encontrado]")
                print(f"{i}. Exercício {num}: {descricao}")
            else:
                print(f"{i}. {item}")
...
def main():
    while True:
        escolha_menu = exibirMenu("MENU PRINCIPAL", list(menus.keys()))
        ...
        while True:
            escolha_exercicio = exibirMenu(f"{escolha_menu} - Exercícios", submenu, is_submenu=True)
            ...
            while True:
                ...
                print("\n" + "="*50)
                print(f"Executando Exercício {numero}")
                print(f"{descricao}\n")
                print("-"*50)
                ...
                repetir = input("\nDeseja repetir o exercício? (s = sim / qualquer tecla = voltar): ").strip().lower()
                if repetir != 's':
                    break
